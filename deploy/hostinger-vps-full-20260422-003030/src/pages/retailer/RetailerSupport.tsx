...
const RetailerSupport = () => {
  ...
  const contactPhone =
    settings?.contactPhone || settings?.supportPhone || "(11) 99999-9999";
  const contactPhoneRaw =
    validation.cleanWhatsAppNumber(
      settings?.contactPhoneRaw || settings?.contactPhone || settings?.supportPhone || "5511999999999"
    ) || "5511999999999";
  const contactEmail =
    settings?.contactEmail || settings?.supportEmail || "suporte@donassistec.com.br";
  const whatsappRawNumber =
    settings?.contactWhatsApp ||
    settings?.whatsappNumber ||
    settings?.contactPhoneRaw ||
    settings?.supportPhone ||
    "5511999999999";
  const whatsappNumber = validation.cleanWhatsAppNumber(whatsappRawNumber);
  ...
  const handleWhatsApp = () => {
    ...
    window.open(
      validation.generateWhatsAppUrl(whatsappRawNumber, whatsappMessage),
      "_blank",
      "noopener,noreferrer"
    );
  };
